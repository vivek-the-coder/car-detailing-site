...
export default function Navbar() {
  ...
  return (
    <>
      ...
      {/* Mobile Menu Overlay - Higher Stack Level */}
      <AnimatePresence mode="wait">
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-50 metal-blur px-8 pt-32 pb-12 flex flex-col xl:hidden"
          >
            {/* Texture Layer */}
            <div className="absolute inset-0 noise-texture opacity-[0.03] pointer-events-none" />

            <div className="flex flex-col gap-6 md:gap-8 container-lux relative z-10 w-full overflow-y-auto">
              {navLinks.map((link, i) => (
                <motion.div
                  key={link.name}
            ...
            <motion.div
              ...
              className="mt-auto container-lux relative z-10 w-full"
            >
              <Link
                href="/book"
                onClick={(e) => handleNavClick(e, "/book")}
                className="w-full py-6 md:py-8 block text-center rounded-2xl border border-luxury-accent/20 bg-luxury-accent/5 text-luxury-accent font-bold uppercase tracking-[0.3em] text-[11px] md:text-[13px] hover:bg-luxury-accent hover:text-black transition-all shadow-[0_0_30px_rgba(0,255,65,0.1)] active:scale-95"
              >
